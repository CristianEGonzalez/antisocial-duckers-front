...
import { obtenerPublicaciones, obtenerPublicacionesPorUserId } from '../../services/postApi'; 
...
const Perfil = () => {
    ...
    useEffect(() => {
        const cargarPerfilYPublicaciones = async () => {
            ...
            if (!usuarioId) { 
                console.error("No hay usuarioId disponible después de que el contexto terminó de cargar.");
                setError("No se pudo cargar el perfil. Por favor, asegúrate de haber iniciado sesión.");
            ...
            try {
                ...
                publicacionesObtenidas = await obtenerPublicacionesPorUserId(usuarioId); 

                if (publicacionesObtenidas && publicacionesObtenidas.length > 0) { 

                    const todasLasPublicaciones = await obtenerPublicaciones(); 
                    publicacionesObtenidas = todasLasPublicaciones.filter(publicacion => publicacion.userId === usuarioId); 
                } else if (!publicacionesObtenidas || publicacionesObtenidas.length === 0) {
                ...
                setError(err.message || 'No se pudieron cargar las publicaciones. Intenta de nuevo.');
            } finally {
                setContenidoCargando(false);
            }
        };
    ...
    if (cargando || contenidoCargando) { 
        return <div className="container mt-5">Cargando perfil y publicaciones...</div>;
    }
    ...
    if (!datosPerfilAmostrar) {
        return <div className="container mt-5">No se pudo obtener la información del perfil.</div>;
    }
                ...
                {publicaciones.length === 0 ? (
                    ...
                    <div className="row g-4">
                        {publicaciones.map((publicacion) => (
                            <div key={publicacion.id} className="col-md-6 col-lg-4">
                                <Publicacion 
                                    key={publicacion.id}
                                    idPublicacion={publicacion.id}
                                    nickName={publicacion.User ? publicacion.User.nickName : 'Desconocido'} 
                                    titulo={publicacion.title || `Publicación #${publicacion.id}`}
                                    ...
                                    etiquetas={publicacion.Tags ? publicacion.Tags.map(tag => tag.tag) : []} 
                                />
                            </div>
                        ))}
                    </div>
